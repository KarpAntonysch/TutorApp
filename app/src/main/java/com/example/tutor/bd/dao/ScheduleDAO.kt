...
interface ScheduleDAO {
    ...
    // запрос на получение заработанной суммы за период (мес., 6мес., год) от текущего дня
    @Query("SELECT sum(a.price)FROM schedeulTable b LEFT JOIN studentTable a ON a.id=b.studentId WHERE CAST(b.dateWithTime/1000 AS integer) BETWEEN strftime('%s',date('now', :period)) AND strftime('%s','now')")
